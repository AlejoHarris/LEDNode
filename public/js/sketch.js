...
function draw() {
    if (turn != 0) {
        background(0);
        fill(255);
        text('BUSY', windowWidth / 2, windowHeight / 2);
        first = true;
        ...
        for (var i = 0; i < ledX * ledY; i++) {
            var r = (xTemp * floor(w) * 4) + (Math.floor(h) * 4 * Math.floor(i / ledX) * windowWidth) + (Math.round(h / 2) * windowWidth * 4) + (Math.round(w / 2) * 4);
            if (sign) {
                xTemp++;
            } else {
                xTemp--;
        ...
        rect(windowWidth / 2, Math.floor(h / 8), windowWidth, Math.floor(h / 4));
    ...
    if (tempX != mouseX || tempY != mouseY) {
        ...
        if (!fading) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                rotate(Math.PI / 3);
                tint((frameCount / 2) % 360, 100, 100, 0.1);
